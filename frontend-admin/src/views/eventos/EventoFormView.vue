<template>
    <AdminLayout :title="esEdicion ? 'Editar Evento' : 'Nuevo Evento'" eyebrow="Módulos · Eventos">
        <div class="content-header">
            <div>
                <h1 class="content-title">{{ esEdicion ? 'Editar Evento' : 'Nuevo Evento' }}</h1>
                <p class="content-desc">{{ esEdicion ? 'Modifica los datos del evento.' : 'Completa el formulario para publicar un nuevo evento.' }}</p>
            </div>
            <RouterLink to="/eventos" class="btn-back">
        ...
        <div v-if="loadingData" class="estado-msg">
            <i class="ti ti-loader-2 spin"></i> Cargando evento...
        </div>
                    ...
                    <div class="card-body">
                        <div class="field">
                            ...
                            <input v-model="form.title" type="text" class="input"
                                placeholder="Escribe el título completo del evento" required />
                        </div>
                        <div class="field">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div v-for="(img, i) in form.gallery" :key="i" class="gallery-item">
                            ...
                            <button type="button" class="btn-remove" @click="removeGallery(i)">
                                <i class="ti ti-x"></i>
                            </button>
                        </div>
                    </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="field">
                            ...
                            <small>Para la lista de eventos (800x600px recomendado)</small>
                        </div>
                        <div class="field" style="margin-top: 16px;">
                            ...
                            <small>Para el detalle del evento (1920x1080px recomendado)</small>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-body">
                        <button type="submit" class="btn-primary btn-full" :disabled="guardando">
                            ...
                            {{ guardando ? 'Guardando...' : (esEdicion ? 'Actualizar evento' : 'Publicar evento') }}
                        </button>
                        <RouterLink to="/eventos" class="btn-secondary btn-full"
...
function convertToEmbed(url: string): string {
    ...
    const fbMatch = url.match(/(?:facebook\.com\/.*\/videos\/(\d+)|fb\.watch\/([a-zA-Z0-9_-]+))/)
    if (fbMatch) return `https://www.facebook.com/plugins/video.php?href=${encodeURIComponent(url)}&show_text=false&width=734`
    return url
}
...
async function guardar() {
    ...
    try {
        ...
        const endpoint = esEdicion.value ? '/events/update-event.php' : '/events/create-event.php'
        const res = await api.post(endpoint, payload)

        if (res.data.success) {
            showToast(esEdicion.value ? 'Evento actualizado' : 'Evento publicado')
...
onMounted(async () => {
    ...
    try {
        const res = await api.get(`/events/get-event.php?id=${route.params.id}`)
        if (res.data.success) {
            ...
            form.value = {
                ...
                videoUrl: e.videoUrl || '',
                content: e.content?.length ? e.content : [''],
                gallery: e.gallery || [],
            }
        }
    } catch { }
