...
try {
    ...
    $check = $conn->prepare("SELECT id FROM eventos WHERE id = ?");
    ...
    $stmt = $conn->prepare(
        "UPDATE eventos SET title=?, short_description=?, date=?, category=?, category_label=?,
    ...
    $del = $conn->prepare("DELETE FROM evento_content WHERE evento_id = ?");
    $del->bind_param("i", $eventoId);
    $del->execute();
    $del->close();

    if (!empty($data['content']) && is_array($data['content'])) {
        $contentStmt = $conn->prepare(
            "INSERT INTO evento_content (evento_id, paragraph_order, paragraph_text) VALUES (?, ?, ?)"
        );
        foreach ($data['content'] as $index => $paragraph) {
            $p = trim($paragraph);
            if (!empty($p)) {
                $order = $index + 1;
                $contentStmt->bind_param("iis", $eventoId, $order, $p);
                $contentStmt->execute();
            }
        }
        $contentStmt->close();
    ...
    $delG = $conn->prepare("DELETE FROM evento_gallery WHERE evento_id = ?");
    $delG->bind_param("i", $eventoId);
    $delG->execute();
    $delG->close();

    if (!empty($data['gallery']) && is_array($data['gallery'])) {
        $galleryStmt = $conn->prepare(
            "INSERT INTO evento_gallery (evento_id, image_url, image_order) VALUES (?, ?, ?)"
        );
        foreach ($data['gallery'] as $index => $imageUrl) {
            $url = filter_var(trim($imageUrl), FILTER_SANITIZE_URL);
            if (!empty($url)) {
                $order = $index + 1;
                $galleryStmt->bind_param("isi", $eventoId, $url, $order);
                $galleryStmt->execute();
            }
        }
        $galleryStmt->close();
    ...
    $action = 'EVENTO_UPDATED';
    $details = "Evento actualizado: ID $eventoId - $title";
    ...
    echo json_encode(['success' => true, 'message' => 'Evento actualizado exitosamente'], JSON_UNESCAPED_UNICODE);
