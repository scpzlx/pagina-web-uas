...
(function () {
    ...
    // Determina la URL base relativa a este script y auto-inyecta el CSS
    // Esto permite incluir solo el <script src="js/chat-widget.js"></script>
    // en cualquier página y que el widget encuentre sus assets y API.
    var categorias = [], subcategorias = [], preguntas = [];
    // localizar el elemento <script> de este archivo (soporta document.currentScript y fallback)
    var _scriptEl = document.currentScript || (function () {
        var ss = document.getElementsByTagName('script');
        for (var i = ss.length - 1; i >= 0; i--) {
            var s = ss[i];
            if (s.src && s.src.indexOf('chat-widget.js') !== -1) return s;
        }
        return null;
    })();

    var _scriptUrl = _scriptEl && _scriptEl.src ? new URL(_scriptEl.src, location.href) : new URL('js/chat-widget.js', location.href);
    // API por defecto: ../api/backend.php relativo a la ubicación del script
    var API = new URL('../api/backend.php', _scriptUrl).href;

    // Auto-inyectar CSS si no existe ya una referencia a chat-widget.css
    (function injectCssIfMissing() {
        try {
            var cssHref = new URL('../css/chat-widget.css', _scriptUrl).href;
            var links = document.querySelectorAll('link[rel="stylesheet"]');
            var found = false;
            for (var i = 0; i < links.length; i++) {
                var href = links[i].href || '';
                if (href.indexOf('chat-widget.css') !== -1 || href === cssHref) { found = true; break; }
            }
            if (!found) {
        ...
        } catch (e) { console.warn('Chat widget: no se pudo inyectar CSS', e); }
    })();
